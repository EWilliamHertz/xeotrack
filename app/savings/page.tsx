...
import { Plus, PiggyBank, Target, ArrowRight, Settings2, Trash2, X, Gift, DollarSign, LineChart as ChartIcon, List, Calendar } from 'lucide-react'
import { fetchSavings, createSavingsAccount, updateSavingsAccount, deleteSavingsAccount, logSavingsTransaction, fetchSavingsLogs, editSavingsTransaction, deleteSavingsTransaction } from './actions'
...
export default function SavingsPage() {
  ...
  const [actionForm, setActionForm] = useState({ amount: '', description: '', date: new Date().toISOString().split('T')[0] })
  ...
  const resetTxForm = () => {
    setActionType(null)
    setEditingTx(null)
    setActionForm({ amount: '', description: '', date: new Date().toISOString().split('T')[0] })
  }
  ...
  const handleTransactionSubmit = async (e: any) => {
    ...
    if (editingTx) {
      ...
      const res = await editSavingsTransaction(localStorage.getItem('token') || '', editingTx.id, viewedAcc.id, Number(actionForm.amount), actionForm.description, actionForm.date)
      if (res.success) refreshAccountData()
    } else {
      // Create new transaction
      const res = await logSavingsTransaction(localStorage.getItem('token') || '', viewedAcc.id, actionType, Number(actionForm.amount), actionForm.description, actionForm.date)
      if (res.success) refreshAccountData()
    }
  }

  const handleDeleteTx = async () => {
    if (!editingTx || !viewedAcc) return
    const res = await deleteSavingsTransaction(localStorage.getItem('token') || '', editingTx.id, viewedAcc.id)
    if (res.success) refreshAccountData()
  }

  const refreshAccountData = async () => {
    resetTxForm()
    // Refresh local history feed
    const logRes = await fetchSavingsLogs(localStorage.getItem('token') || '', viewedAcc.id)
    if (logRes.success) setLogs(logRes.data)
    
    // Refresh Master accounts list to get new balance
    const accRes = await fetchSavings(localStorage.getItem('token') || '')
    if (accRes.success) {
      setAccounts(accRes.data)
      const updatedAcc = accRes.data.find((a: any) => a.id === viewedAcc.id)
      if (updatedAcc) setViewedAcc(updatedAcc)
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] p-8 space-y-4 shadow-2xl relative">
            ...
            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 py-4 rounded-2xl font-bold shadow-lg mt-4 text-white">Save Account</button>
          </form>
        </div>
      )}
      ...
      {viewedAcc && (
        <div className="fixed inset-0 z-[200] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] p-6 space-y-4 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            {isEditing ? (
              <div className="space-y-4 bg-slate-950 p-4 rounded-xl border border-slate-800">
                ...
                <div className="space-y-2">
                  ...
                  <input value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-3 rounded-lg text-white outline-none focus:border-emerald-500" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-slate-500 font-bold tracking-widest">TARGET GOAL (SEK)</label>
                  <input type="number" value={editForm.target_amount} onChange={e => setEditForm({...editForm, target_amount: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-3 rounded-lg font-mono text-white outline-none focus:border-emerald-500" />
                </div>
                <div className="flex gap-2 pt-2">
                  <button onClick={handleUpdateAccount} className="flex-1 bg-emerald-600 hover:bg-emerald-500 py-3 rounded-lg font-bold text-white">Save Changes</button>
                  <button onClick={handleDeleteAccount} className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 px-4 py-3 rounded-lg transition"><Trash2 size={20} /></button>
                </div>
              </div>
            ) : (
              <>
                ...
                {actionType && (
                  <form onSubmit={handleTransactionSubmit} className="bg-slate-900 border border-emerald-500/50 p-5 rounded-2xl space-y-4 shadow-lg relative">
                    <div className="flex justify-between items-center">
                      <p className="text-sm font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-2">
                        {editingTx ? 'Edit Transaction' : actionType === 'in-kind' ? <><Gift size={16}/> Donate Asset</> : actionType === 'deposit' ? 'Cash Deposit' : 'Cash Withdrawal'}
                      </p>
                      <button type="button" onClick={resetTxForm} className="text-slate-500 hover:text-white"><X size={18}/></button>
                    </div>
                    
                    <input required type="number" placeholder={actionType === 'in-kind' ? "Appreciated Value (SEK)" : "Amount (SEK)"} value={actionForm.amount} onChange={e => setActionForm({...actionForm, amount: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-mono text-xl text-white outline-none focus:border-emerald-500" />
                    
                    <div className="grid grid-cols-1 gap-3">
                      <input placeholder={actionType === 'in-kind' ? "Asset Name (e.g. Machinery, Artwork)" : "Reason/Reference (Optional)"} value={actionForm.description} onChange={e => setActionForm({...actionForm, description: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-sm text-white outline-none focus:border-emerald-500" />
                      <div className="flex items-center gap-2 bg-slate-950 p-3 rounded-xl border border-slate-800 focus-within:border-emerald-500">
                        <Calendar size={16} className="text-slate-500"/>
                        <input required type="date" className="bg-transparent text-white outline-none w-full text-sm" value={actionForm.date} onChange={e => setActionForm({...actionForm, date: e.target.value})} />
                      </div>
                    </div>

                    <div className="flex gap-2 pt-2">
                      <button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-500 py-3 rounded-xl text-sm font-bold text-white">
                        {editingTx ? 'Update Entry' : 'Confirm'}
                      </button>
                      {editingTx && (
                        <button type="button" onClick={handleDeleteTx} className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 px-4 rounded-xl transition">
                          <Trash2 size={20} />
                        </button>
                      )}
                    </div>
                        ...
                        {logs.length === 0 ? <p className="text-center text-slate-500 text-sm py-4">No transactions yet.</p> : logs.slice().reverse().map((log: any) => (
                          <div 
                            ...
                            onClick={() => {
                              ...
                              setActionForm({ amount: log.amount, description: log.description || '', date: new Date(log.date).toISOString().split('T')[0] })
                            }}
                            className="cursor-pointer flex justify-between items-center text-sm bg-slate-950 p-3 rounded-lg border border-slate-800/50 hover:border-emerald-500/50 transition group"
                          >
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {log.description && <span className="text-slate-300 text-xs truncate max-w-[120px]">{log.description}</span>}
                              </div>
                              <span className="text-slate-500 text-[10px] mt-1.5 block font-mono">{new Date(log.date).toLocaleDateString()}</span>
                            </div>
